...
class HeaderLayoutColorsCest
{
    ...
    public function tryToTest(CustomizerTester $I, AdminLogin $AdminLoginPage, Customizer $CustomizerPage, Home $HomaPage)
    {
        ...
        $siteTitleColor = $I->executeInSelenium(function(\Facebook\WebDriver\Remote\RemoteWebDriver $webdriver){
            return $webdriver->findElement(WebDriverBy::cssSelector('.site-title > a'))->getCSSValue('color');
        });
        ...
        $I->wait(1);
        $I->fillField($CustomizerPage->customizer_header_colors_Border_Color_Input,'#987654');
        $I->wait(1);

        $I->click($CustomizerPage->customizer_header_colors_SiteTitle_Color);
        $I->wait(1);
        $I->fillField($CustomizerPage->customizer_header_colors_SiteTitle_Color_Input,'#654789');
        $I->wait(1);

        $I->click($CustomizerPage->customizer_header_colors_SiteTagline_Color);
        $I->wait(1);
        $I->fillField($CustomizerPage->customizer_header_colors_SiteTagline_Color_Input,'#123456');
        $I->wait(1);
        ...
        $I->assertEquals('rgba(152, 118, 84, 1)',$headerBorderColor);

        $siteTitleColor = $I->executeInSelenium(function(\Facebook\WebDriver\Remote\RemoteWebDriver $webdriver){
            return $webdriver->findElement(WebDriverBy::cssSelector('.site-title > a'))->getCSSValue('color');
        });
        $I->assertEquals('rgba(101, 71, 137, 1)',$siteTitleColor);

        $siteTaglineColor = $I->executeInSelenium(function(\Facebook\WebDriver\Remote\RemoteWebDriver $webdriver){
            return $webdriver->findElement(WebDriverBy::cssSelector('.site-description'))->getCSSValue('color');
        });
        $I->assertEquals('rgba(18, 52, 86, 1)',$siteTaglineColor);
        ...
        $I->amGoingTo('Check Updated Header Menu Colors without customizer');
        $I->amOnPage($HomaPage->url);
