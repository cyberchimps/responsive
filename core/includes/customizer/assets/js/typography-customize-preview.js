...
( function( $ ) {
    ...
    } ), api( "add_to_cart_typography[font-family]", function( $swipe ) {
        $swipe.bind( function( pair ) {
            if ( pair ) {
                /** @type {string} */
                var fontName = pair.split(",")[0];
                fontName = fontName.replace(/'/g, '');
                var idfirst = ( fontName.trim().toLowerCase().replace( " ", "-" ), "customize-control-add_to_cart_typography-font-family" );
                var fontSize = fontName.replace( " ", "%20" );
                fontSize = fontSize.replace( ",", "%2C" );
                /** @type {string} */
                fontSize = responsive.googleFontsUrl + "/css?family=" + fontName + ":" + responsive.googleFontsWeight;
            ...
            jQuery( 'style.customizer-typography-add_to_cart-font-family' ).remove();
            jQuery( 'head' ).append(
                '<style class="customizer-typography-add_to_cart-font-family">'
                +  responsive.selectorArray['add_to_cart'] + '{ font-family:' + pair +';}'
                + '</style>'
            );
        } );
    } ), api( "add_to_cart_typography[font-weight]", function( $swipe ) {
        $swipe.bind( function( dataAndEvents ) {
            jQuery( 'style.customizer-typography-add_to_cart_typography-font-weight' ).remove();
            jQuery( 'head' ).append(
                '<style class="customizer-typography-add_to_cart_typography-font-weight">'
                +  responsive.selectorArray['add_to_cart'] + '{ font-weight:' + dataAndEvents +' !important;}'
                + '</style>'
            );

        } );
    } ), api( "add_to_cart_typography[font-style]", function( $swipe ) {
        $swipe.bind( function( dataAndEvents ) {
            jQuery( 'style.customizer-typography-add_to_cart_typography-font-style' ).remove();
            jQuery( 'head' ).append(
                '<style class="customizer-typography-add_to_cart_typography-font-style">'
                +  responsive.selectorArray['add_to_cart'] + '{ font-style:' + dataAndEvents +';}'
                + '</style>'
            );

        } );
    } ), api( "add_to_cart_typography[font-size]", function( $swipe ) {
        $swipe.bind( function( dataAndEvents ) {
            jQuery( 'style.customizer-typography-add_to_cart_typography-font-size' ).remove();
            jQuery( 'head' ).append(
                '<style class="customizer-typography-add_to_cart_typography-font-size">'
                +  responsive.selectorArray['add_to_cart'] + '{ font-size:' + dataAndEvents +' !important;}'
                + '</style>'
            );

        } );
    } ), api( "add_to_cart_tablet_typography[font-size]", function( $swipe ) {
        $swipe.bind( function( dataAndEvents ) {
            jQuery( 'style.customizer-typography-add_to_cart_typography-tablet-font-size' ).remove();
            jQuery( 'head' ).append(
                '<style class="customizer-typography-add_to_cart_typography-tablet-font-size">'
                + '@media (max-width: 768px){'+ responsive.selectorArray['add_to_cart'] + '{ font-size:' + dataAndEvents +' !important;}}'
                + '</style>'
            );
        } );
    } ), api( "add_to_cart_mobile_typography[font-size]", function( $swipe ) {
        $swipe.bind( function( dataAndEvents ) {
            jQuery( 'style.customizer-typography-add_to_cart_typography-mobile-font-size' ).remove();
            jQuery( 'head' ).append(
                '<style class="customizer-typography-add_to_cart_typography-mobile-font-size">'
                + '@media (max-width: 480px){'+ responsive.selectorArray['add_to_cart'] + '{ font-size:' + dataAndEvents +' !important;}}'
                + '</style>'
            );

        } );
    } ), api( "add_to_cart_typography[line-height]", function( $swipe ) {
        $swipe.bind( function( dataAndEvents ) {
            jQuery( 'style.customizer-typography-add_to_cart_typography-line-height' ).remove();
            jQuery( 'head' ).append(
                '<style class="customizer-typography-add_to_cart_typography-line-height">'
                +  responsive.selectorArray['add_to_cart'] + '{ line-height:' + dataAndEvents +' !important;}'
                + '</style>'
            );

        } );
    } ), api( "add_to_cart_typography[letter-spacing]", function( $swipe ) {
        $swipe.bind( function( dataAndEvents ) {
            jQuery( 'style.customizer-typography-add_to_cart_typography-letter-spacing' ).remove();
            jQuery( 'head' ).append(
                '<style class="customizer-typography-add_to_cart_typography-letter-spacing">'
                +  responsive.selectorArray['add_to_cart'] + '{ letter-spacing:' + dataAndEvents +'px;}'
                + '</style>'
            );

        } );
    } ), api( "add_to_cart_typography[text-transform]", function( $swipe ) {
        $swipe.bind( function( dataAndEvents ) {
            jQuery( 'style.customizer-typography-add_to_cart_typography-text-transform' ).remove();
            jQuery( 'head' ).append(
                '<style class="customizer-typography-add_to_cart_typography-text-transform">'
                +  responsive.selectorArray['add_to_cart'] + '{ text-transform:' + dataAndEvents +';}'
                + '</style>'
            );
