<?php
/**
 * Customizer Control: responsive-unit-dimensions.
 *
...
if ( ! class_exists( 'Responsive_Customizer_Unit_Dimensions_Control' ) ) :
	/**
	 * Unit Dimensions control
	 */
	class Responsive_Customizer_Unit_Dimensions_Control extends WP_Customize_Control {
		/**
		 * The control type.
		 *
		 * @access public
		...
		public function enqueue() {
			wp_localize_script( 'responsive-unit-dimensions', 'responsiveL10n', $this->l10n() );
			// We can reuse the same CSS or create a new one. Let's reuse dimensions CSS and add custom if needed.
		...
		public function to_json() {
			...
			foreach ( $this->settings as $setting_key => $setting ) {
				if ( strpos( $setting_key, '_unit' ) !== false ) {
					// Handle unit settings: desktop_unit, tablet_unit, mobile_unit
					$this->json['units'][ $setting_key ] = array(
						'id'    => $setting->id,
						'link'  => $this->get_link( $setting_key ),
						'value' => $this->value( $setting_key ),
					);
				} else {
					// Handle dimension settings: desktop_top, tablet_top, etc.
					list( $_key ) = explode( '_', $setting_key );

					if ( in_array( $_key, array( 'desktop', 'tablet', 'mobile' ) ) ) {
