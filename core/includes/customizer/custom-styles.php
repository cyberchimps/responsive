...
function responsive_customizer_styles() {
	...
	// Content Left/Right Edge Spacing & Content Top/Bottom Spacing
	...
	if ( 'boxed' === $responsive_blog_container_style ) {
		...
		// Archive Grid Boxed Border Radius.
		$archive_grid_boxed_radius              = get_theme_mod( 'responsive_archive_grid_boxed_radius', '' );
		$archive_grid_boxed_radius_unit         = get_theme_mod( 'responsive_archive_grid_boxed_radius_unit', 'px' );
		$archive_grid_boxed_radius_tablet      = get_theme_mod( 'responsive_archive_grid_boxed_radius_tablet', '' );
		$archive_grid_boxed_radius_tablet_unit = get_theme_mod( 'responsive_archive_grid_boxed_radius_tablet_unit', 'px' );
		$archive_grid_boxed_radius_mobile      = get_theme_mod( 'responsive_archive_grid_boxed_radius_mobile', '' );
		$archive_grid_boxed_radius_mobile_unit = get_theme_mod( 'responsive_archive_grid_boxed_radius_mobile_unit', 'px' );
		...
		if ( '' !== $archive_grid_boxed_radius ) {
			$custom_css .= "
			.blog.responsive-site-style-content-boxed:not(.custom-home-page-active) .site-content .hentry,
			.blog.responsive-site-style-boxed:not(.custom-home-page-active) .site-content .hentry,
			.archive.responsive-site-style-content-boxed:not(.post-type-archive-product) .site-content .hentry,
			.archive.responsive-site-style-boxed:not(.post-type-archive-product) .site-content .hentry {
				border-radius: {$archive_grid_boxed_radius}{$archive_grid_boxed_radius_unit};
			}";
		}
		$custom_css .= "
		@media screen and ( max-width: 992px ) {
		...
		if ( '' !== $archive_grid_boxed_radius_tablet ) {
			$custom_css .= "
			.blog.responsive-site-style-content-boxed:not(.custom-home-page-active) .site-content .hentry,
			.blog.responsive-site-style-boxed:not(.custom-home-page-active) .site-content .hentry,
			.archive.responsive-site-style-content-boxed:not(.post-type-archive-product) .site-content .hentry,
			.archive.responsive-site-style-boxed:not(.post-type-archive-product) .site-content .hentry {
				border-radius: {$archive_grid_boxed_radius_tablet}{$archive_grid_boxed_radius_tablet_unit};
			}";
		} elseif ( '' !== $archive_grid_boxed_radius ) {
			$custom_css .= "
			.blog.responsive-site-style-content-boxed:not(.custom-home-page-active) .site-content .hentry,
			.blog.responsive-site-style-boxed:not(.custom-home-page-active) .site-content .hentry,
			.archive.responsive-site-style-content-boxed:not(.post-type-archive-product) .site-content .hentry,
		...
		if ( '' !== $archive_grid_boxed_radius_mobile ) {
			$custom_css .= "
			.blog.responsive-site-style-content-boxed:not(.custom-home-page-active) .site-content .hentry,
			.blog.responsive-site-style-boxed:not(.custom-home-page-active) .site-content .hentry,
			.archive.responsive-site-style-content-boxed:not(.post-type-archive-product) .site-content .hentry,
			.archive.responsive-site-style-boxed:not(.post-type-archive-product) .site-content .hentry {
				border-radius: {$archive_grid_boxed_radius_mobile}{$archive_grid_boxed_radius_mobile_unit};
			}";
		} elseif ( '' !== $archive_grid_boxed_radius ) {
			$custom_css .= "
			.blog.responsive-site-style-content-boxed:not(.custom-home-page-active) .site-content .hentry,
...
.mobile-header-widgets .mobile-header-widgets-wrapper a:hover { color: {$mobile_header_widget_link_hover_color}; }";
	...
	if ( ! class_exists( 'Responsive_Addons_Pro' ) ) {
		...
		$single_blog_category_color = esc_html(
			get_theme_mod(
				'responsive_single_blog_category_color_color',
				Responsive\Core\get_responsive_customizer_defaults( 'responsive_single_blog_category_color' )
			)
		);

		$custom_css .= "
		.single.single-post .entry-category a {
			color: {$single_blog_category_color};
		}";

		// Single Post — Category Font.
		$single_blog_category_typography = get_theme_mod( 'single_blog_category_typography' );
		if ( $single_blog_category_typography && is_array( $single_blog_category_typography ) ) {
			$custom_css .= ".single.single-post .entry-category a {";
			foreach ( $single_blog_category_typography as $prop => $val ) {
				if ( $val ) {
					$custom_css .= $prop . ': ' . $val . ';';
				}
			}
		...
		$single_blog_item_meta_color       = esc_html(
			get_theme_mod(
				'responsive_single_blog_item_meta_color',
				Responsive\Core\get_responsive_customizer_defaults( 'responsive_single_blog_item_meta_color' )
			)
		);
		...
		$custom_css .= "
		.single.single-post .post-meta .entry-author a,
		.single.single-post .post-meta .entry-date a {
			color: {$single_blog_item_meta_color};
		}
		.single.single-post .post-meta .entry-author a:hover,
		.single.single-post .post-meta .entry-date a:hover {
			color: {$single_blog_item_meta_hover_color};
		}";
		...
		$single_blog_excerpt_typography = get_theme_mod( 'single_blog_excerpt_typography' );
		if ( $single_blog_excerpt_typography && is_array( $single_blog_excerpt_typography ) ) {
			$custom_css .= ".single.single-post .entry-content {";
			foreach ( $single_blog_excerpt_typography as $prop => $val ) {
				if ( $val ) {
					$custom_css .= $prop . ': ' . $val . ';';
				}
			}
		...
		$single_blog_excerpt_tablet_typography = get_theme_mod( 'single_blog_excerpt_tablet_typography' );
		if ( $single_blog_excerpt_tablet_typography && is_array( $single_blog_excerpt_tablet_typography ) ) {
			$custom_css .= "@media screen and ( max-width: 992px ) {";
			$custom_css .= ".single.single-post .entry-content {";
			foreach ( $single_blog_excerpt_tablet_typography as $prop => $val ) {
				if ( $val ) {
					$custom_css .= $prop . ': ' . $val . ';';
				}
			}
		...
		$single_blog_excerpt_mobile_typography = get_theme_mod( 'single_blog_excerpt_mobile_typography' );
		if ( $single_blog_excerpt_mobile_typography && is_array( $single_blog_excerpt_mobile_typography ) ) {
			$custom_css .= "@media screen and ( max-width: 576px ) {";
			$custom_css .= ".single.single-post .entry-content {";
			foreach ( $single_blog_excerpt_mobile_typography as $prop => $val ) {
				if ( $val ) {
					$custom_css .= $prop . ': ' . $val . ';';
				}
			}
